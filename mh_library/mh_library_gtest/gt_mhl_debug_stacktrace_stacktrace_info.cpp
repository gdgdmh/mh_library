        ...
        namespace stacktrace
        {
            namespace stacktrace_info
            {
                ...
                TEST(Debug_Stacktrace_StacktraceInfo, GetSize1)
                {
                    ...
                    std::vector<StacktraceInfo::FileName> file_names;
                    std::vector<StacktraceInfo::LineNumber> line_numbers;
                    StacktraceInfo s(kSize, std::move(addresses), std::move(symbols),
                                     std::move(file_names), std::move(line_numbers));
                    EXPECT_EQ(s.GetSize(), kSize);
                }
                ...
                TEST(Debug_Stacktrace_StacktraceInfo, GetAddress2)
                {
                    ...
                    std::vector<StacktraceInfo::FileName> file_names;
                    std::vector<StacktraceInfo::LineNumber> line_numbers;
                    addresses.emplace_back(static_cast<void *>(nullptr));
                    addresses.emplace_back(nullptr);
                    StacktraceInfo s(kSize, std::move(addresses), std::move(symbols),
                                     std::move(file_names), std::move(line_numbers));
                    EXPECT_EQ(s.GetAddresses().size(), kSize);
                    EXPECT_EQ(s.GetAddresses().at(0), static_cast<void*>(nullptr));
