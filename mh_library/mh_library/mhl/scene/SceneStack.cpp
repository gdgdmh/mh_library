 ...
 */
void mhl::SceneStack::Push(const std::shared_ptr<ISceneable> &scene)
{
    if (!stack_.empty())
    {
        // pushされるときのイベントを通知
 ...
 */
void mhl::SceneStack::Swap(const std::shared_ptr<ISceneable> &scene)
{
    if (stack_.empty())
    {
        return;
...
void mhl::SceneStack::InitializeScene(
    const std::shared_ptr<ISceneable> &scene)
{
    scene->Initialize();
    scene->Load();
}
 ...
 * @param scene 対象のシーン
 */
void mhl::SceneStack::EndScene(const std::shared_ptr<ISceneable> &scene)
{
    scene->Release();
}
